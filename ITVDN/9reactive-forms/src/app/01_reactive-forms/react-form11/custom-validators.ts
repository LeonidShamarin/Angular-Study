...
export class userDataValidation{
    static passport(control: AbstractControl /*поле с формы*/): { [key: string]: any }| null {
...
export function passportFunc(control: AbstractControl /*поле с формы*/): { [key: string]: any }| null{
    let passportRegex = /^([A-ZА-Я]{2}[0-9]{6})$|^([0-9]{9})$/;
    // Значение поля с формы
    let value = control.value;
    // Проверка поля по регулярному выражению
